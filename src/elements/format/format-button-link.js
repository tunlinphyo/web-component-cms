...
export class FormatButtonLink extends LitElement {
  static properties = {
  ...
  render() {
    return html`
      <button
        type="button"
        title=${this.applied ? "Edit button link" : "Add button link"}
        aria-label=${this.applied ? "Edit button link" : "Add button link"}
        popovertarget="button-link-editor"
      ...
      <div id="button-link-editor" popover>
  ...
  #dispatch(value) {
    this.dispatchEvent(
      new CustomEvent("format-command", {
        detail: { command: "buttonLink", value },
        bubbles: true,
        composed: true,
      }),
    );
